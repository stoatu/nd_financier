...
function get_chinese_name(en_name) {
    ...
    //计划发布
    if (en_name == "description")
        return "计划概述";
    if (en_name == "plan_name")
        return "计划名称";
    if (en_name == "expect_performance")
        return "目标收益";
    if (en_name == "stop_loss")
        return "止损值";
    if (en_name == "start_time")
        return "开始时间";
    if (en_name == "operate_money")
        return "建议操盘资金";
...
function export_error(formName, object, error_msg) {
    ...
    if (error_type == "right") {
        ...
        object.parent().append("<span class='error_msg'>(" + error_msg + ")</span>");
    }
}
