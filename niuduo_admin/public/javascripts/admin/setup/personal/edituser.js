...
$(document).ready(function() {
    ...
    var  grid_citylist = new Select_Grid("/showAllRole",$("#role"),columns_city,"");

    grid_citylist.default_value=parent.temp_user_obj.role_id;
    grid_citylist.Init("");


    $("#login_name").val(parent.temp_user_obj.login_name);
    $("#real_name").val(urldecode(parent.temp_user_obj.real_name));
    $("#user_id").val(parent.temp_user_obj.user_id);
    $("#thumbnail").val(parent.temp_user_obj.img_url);
    $("#mobile").val(parent.temp_user_obj.mobile);
    $("#nick_name").val(parent.temp_user_obj.nick_name);
    $("#positive_file_pic").attr("src",parent.temp_user_obj.img_url);

    //alert(parent.temp_user_obj.role_id);
    //grid_citylist.select_by_value(parent.temp_user_obj.role_id);
    get_sign();
    ...
    $(".btn_save").bind("click",function(){
        $("#add_form").attr("action","/adminEditNameAndRole");
        if(status==1)
            return;
